...
Currently configured project: `codex-translator-chrome-extension` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/codex-translator-chrome-extension/SETUP.md` to fill in
   `data/codex-translator-chrome-extension/INFO.md`.
...
`--project-id codex-translator-chrome-extension` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/codex-translator-chrome-extension/`.
...
data/codex-translator-chrome-extension/
